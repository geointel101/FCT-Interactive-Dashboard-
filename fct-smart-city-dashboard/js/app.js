    ...
    layers: [
      ...
      },
      {
        ...
        paint: {
          ...
          "circle-stroke-color": "#5a4a35",
          "circle-stroke-width": 0.5,
        },
      },

      // Building footprints (only present at zoom 13+ in the tileset)
      {
        id: "buildings",
        type: "fill",
        source: "fct",
...
// Each entry maps a layer to a title field + rows of [label, propertyKey]
const FACILITY_POPUPS = {
  schools: {
    title: (p) => p.School_nam || "School",
    rows: [
      ["Category", "category"],
  ...
  health_facilities: {
    title: (p) => p.name || "Health Facility",
    rows: [
      ["Type", "type"],
      ["Category", "category"],
  ...
  water_facilities: {
    title: (p) => p.Water_Type || "Water Facility",
    rows: [
      ["Community", "Community"],
      ["Ward", "Ward"],
      ["Improved", "is_improve"],
  ...
  service_centers: {
    title: (p) => p.name || "Service Center",
    rows: [["Type", "type"]],
  },
  settlements: {
    title: (p) => p.NAME || "Settlement",
    rows: [],
  },
};

Object.keys(FACILITY_POPUPS).forEach((layerId) => {
  const config = FACILITY_POPUPS[layerId];
  map.on("click", layerId, (e) => {
    const f = e.features[0];
    const p = f.properties;
    const rowsHtml = config.rows
      .filter(([, key]) => p[key] !== undefined && p[key] !== null && p[key] !== "")
      .map(([label, key]) => `${label}: ${p[key]}`)
      .join("<br>");
    new maplibregl.Popup({ offset: 8 })
      .setLngLat(e.lngLat)
      .setHTML(
        `<div class="popup-title">${config.title(p)}</div>` +
          (rowsHtml ? `<div class="popup-meta">${rowsHtml}</div>` : "")
      )
      .addTo(map);
  });
  ...
  new maplibregl.Popup({ offset: 8 })
    ...
    .setHTML(
      `<div class="popup-title">${p.Ward_Name || "Ward"}</div>` +
        `<div class="popup-meta">LGA: ${p.LGA_Name || "—"}<br>Status: ${
          p.Status || "—"
        }<br>Urban: ${p.Urban || "—"}</div>`
    )
    .addTo(map);
});
